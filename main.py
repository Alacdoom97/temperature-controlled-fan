...
# Read DHT sensor data and publish it to MQTT broker
def read_dht_and_publish():
    dht_sensor = dht.DHT11(machine.Pin(27))
    dht_sensor.measure()
    temperature = dht_sensor.temperature()
    humidity = dht_sensor.humidity()
    print("Temperature is {} degrees Celsius, Humidity is {}%".format(temperature, humidity,))
    payload = "Temperature: {:.2f}°C, Humidity: {:.2f}%".format(temperature, humidity)

    client = MQTTClient("pico_w_client", mqtt_broker, mqtt_port)
...
while True:
    ...
    temperature = tempSensor.temperature()
    humidity = tempSensor.humidity()
    if temperature >= 30 and checkTick == 1:
        continue
    elif temperature >= 30 and checkTick == 0:
        playsong(songStart)
        checkTick = 1
    elif temperature < 30 and checkTick == 0:
        continue
    elif temperature < 30 and checkTick == 1:
        playsong(songStop)
        checkTick = 0
    else:
        continue
    print("Temperature is {} degrees Celsius, Humidity is {}% and checkTick is {}".format(temperature, humidity, checkTick))
    time.sleep(2)"""
